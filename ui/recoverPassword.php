...
if(isset($_POST['recover'])){
	...
	if(!$foundEmail){
		$recoverClientes = new Clientes();
		if($recoverClientes -> existEmail($_POST['email'])) {;
			$generatedPassword = rand(100000,999999);
			$recoverClientes -> recoverPassword($_POST['email'], $generatedPassword);
		$foundEmail = true;
		}
	}
	if($foundEmail){
		$to=$_POST['email'];
		$subject="Recuperación de clave para lt";
		$from="FROM: lt <contact@itiud.org>";
